...
3. **Ola 3 IA — captura por voz + foto→macros con Claude (PUSHEADO, 2026-07-07).** `ANTHROPIC_API_KEY` seteada en Vercel (Sensitive, Production + Preview). Modelo: **`claude-sonnet-4-6`** (por costo). Deployado: `api/parse.js` (voz/texto → módulo), `api/foto.js` (foto del plato → alimentos+gramos), `package.json` (raíz), `js/core/captura.js` cableado (AI-first con fallback determinístico + botón 📷). **Probar en prod (tras hard-refresh):** voz/texto — "gasté 5 lucas en el súper", "almorcé 250 de carne y 2 huevos", "press banca 4x10 con 80", "tomé la creatina"; y 📷 sacando/subiendo una foto de un plato. **Si el sitio 404ea tras el deploy**, agregar un `vercel.json` mínimo (el `package.json` cambió cómo Vercel buildea). Próximo: **coach proactivo** en Insights (o lo que priorice Fede tras probar).
