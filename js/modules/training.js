...
async function agregarSet(ejId) {
  ...
  try {
    ...
    const { data, error } = await supabase.from('training_sets').insert(fila).select().single();
    if (error) throw error;
    if (S.fecha === fecha) S.sets.push(data);
    paint();
  } catch (err) {
...
function vistaSesion() {
  ...
  const { volumen: vol } = resumenSesion(S.sets);
  const cuerpo = grupos.length
    ? grupos.map(bloqueEjercicio).join('')
    : `<div class="tra-slot-vacio">Sesión vacía. Agregá el primer ejercicio 👇</div>`;
  return nav + `
  <div class="tra-card tra-sesion-head">
    ...
    <div class="tra-sesion-vol">
      <span class="tra-vol-k">Volumen</span>
      <span class="tra-vol-v"><span class="tra-num" id="traVolTotal">${num0(vol)}</span> ${esc(unidadVolumen())}</span>
    </div>
  </div>
  ${cuerpo}
  <button class="tra-add-ejercicio" data-action="abrir-ejpicker">+ Ejercicio</button>`;
}
...
function bloqueEjercicio(g) {
  ...
  return `
  <section class="tra-card tra-ejercicio">
    <header class="tra-ej-head">
      <div class="tra-ej-info">
        <h3 class="tra-ej-nombre">${esc(ejercicioNombre(g.ejId))}</h3>
...
function modalEjercicios() {
  ...
  <div class="tra-modal" data-action="ejpicker-fondo">
    <div class="tra-modal-card" role="dialog" aria-modal="true" aria-label="Agregar ejercicio">
...
function cardHistorial(item) {
  ...
  return `
  <section class="tra-card tra-hist-item">
...
function vistaProgreso() {
  ...
  const selector = `
  <div class="tra-card tra-prog-selector">
    <label class="tra-field">
      <span class="tra-campo-label">Ejercicio</span>
  ...
  if (S.progresoCargando) return selector + `<div class="tra-cargando">Cargando el progreso…</div>`;
  ...
  return selector + bloqueProgreso(puntos);
}
...
.tra-titulo { margin: 0; font-family: var(--font-display); font-size: 1.35rem; letter-spacing: .01em; }
.tra-tabs { display: flex; gap: var(--space-2); overflow-x: auto; -webkit-overflow-scrolling: touch; }
.tra-tab { flex: 1 1 0; min-height: 44px; padding: var(--space-2) var(--space-3); background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); color: var(--text-dim); white-space: nowrap; transition: background .15s, color .15s, border-color .15s; }
.tra-tab.activa { background: var(--accent-soft); border-color: var(--accent); color: var(--accent); font-weight: 600; }
...
/* Header de sesión */
.tra-sesion-head { display: flex; align-items: center; gap: var(--space-3); flex-wrap: wrap; }
.tra-nombre-input { flex: 1; min-width: 140px; min-height: 44px; padding: var(--space-2) var(--space-3); background: var(--bg); border: 1px solid var(--border-strong); border-radius: var(--radius); color: var(--text); font: inherit; font-family: var(--font-display); font-weight: 600; }
...
.tra-sesion-vol { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; flex: none; }
.tra-vol-k { font-size: .68rem; color: var(--text-faint); text-transform: uppercase; letter-spacing: .06em; }
.tra-vol-v { font-size: 1.05rem; font-weight: 700; color: var(--accent); }
...
.tra-set { display: grid; grid-template-columns: 28px 1fr 1fr 64px 40px; gap: var(--space-2); align-items: center; }
